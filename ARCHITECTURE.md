# Архитектура EngCRM - Адреса и маршрутизация

## 📍 Публичные адреса (для пользователей)

### Frontend
```
https://justspeak.heliad.ru
```
- Главная страница приложения
- Вход в систему
- Все UI компоненты

### Backend API
```
https://justspeak.heliad.ru/api/*
```
- Все API endpoints доступны через префикс `/api`
- Примеры:
  - `https://justspeak.heliad.ru/api/auth/login`
  - `https://justspeak.heliad.ru/api/users`
  - `https://justspeak.heliad.ru/api/lessons`

### Backend Health Check
```
https://justspeak.heliad.ru/health
```
- Проверка состояния backend

### API Документация (Swagger)
```
https://justspeak.heliad.ru/api/docs
```
- Интерактивная документация API

## 🔒 Внутренние адреса (доступны только на сервере)

### Frontend Container
```
127.0.0.1:3005
```
- Nginx контейнер
- Раздаёт статические файлы (HTML, CSS, JS)
- **НЕ доступен** извне (только localhost)

### Backend Container
```
127.0.0.1:8005
```
- FastAPI приложение
- Обрабатывает API запросы
- **НЕ доступен** извне (только localhost)

### PostgreSQL Database
```
127.0.0.1:5435
```
- База данных
- **НЕ доступна** извне (только localhost)

## 🌐 Как работает маршрутизация

### 1. Пользователь открывает сайт

```
Браузер пользователя
    ↓ HTTPS запрос
https://justspeak.heliad.ru
    ↓
Nginx (сервер 158.160.141.83:443)
    ↓ проксирует на
Frontend Container (127.0.0.1:3005)
    ↓ возвращает
index.html + JavaScript файлы
```

### 2. Frontend делает API запрос

**Код в браузере (frontend/src/services/api.ts):**
```typescript
const api = axios.create({
  baseURL: "/api",  // Относительный путь!
});

// Запрос:
await api.get("/users")
```

**Маршрут запроса:**
```
JavaScript в браузере
    ↓ HTTP запрос
https://justspeak.heliad.ru/api/users
    ↓
Nginx (определяет маршрут по префиксу /api)
    ↓ проксирует на
Backend Container (127.0.0.1:8005)
    ↓ FastAPI обрабатывает
GET /api/users → возвращает данные
```

### 3. Загрузка статических файлов

```
Браузер запрашивает
    ↓
https://justspeak.heliad.ru/assets/index-DSNp97S9.js
    ↓
Nginx (префикс не /api, не /health)
    ↓ проксирует на
Frontend Container (127.0.0.1:3005)
    ↓ возвращает
JavaScript файл
```

## 📝 Nginx конфигурация

**Файл:** `/etc/nginx/sites-available/justspeak.heliad.ru`

```nginx
server {
    server_name justspeak.heliad.ru;

    # Backend API
    location /api {
        proxy_pass http://127.0.0.1:8005;
        # Headers для правильной работы
    }

    # Backend Health
    location /health {
        proxy_pass http://127.0.0.1:8005/health;
    }

    # Frontend (всё остальное)
    location / {
        proxy_pass http://127.0.0.1:3005;
    }

    # SSL сертификат
    listen 443 ssl;
    ssl_certificate /etc/letsencrypt/live/justspeak.heliad.ru/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/justspeak.heliad.ru/privkey.pem;
}

# Редирект HTTP -> HTTPS
server {
    listen 80;
    server_name justspeak.heliad.ru;
    return 301 https://$host$request_uri;
}
```

## 🔧 Frontend конфигурация

### Axios baseURL (frontend/src/services/api.ts)
```typescript
const api = axios.create({
  baseURL: "/api",  // Относительный путь!
});
```

**Почему `/api` а не полный URL?**

✅ **Правильно** (относительный путь):
```typescript
baseURL: "/api"
```
- Работает на любом домене
- Браузер автоматически подставит текущий домен
- Запрос: `https://justspeak.heliad.ru/api/users`
- Nginx проксирует на backend

❌ **Неправильно** (абсолютный URL):
```typescript
baseURL: "http://158.160.141.83:8005"
```
- Не будет работать (порт закрыт)
- Проблемы с CORS
- Не использует SSL

## 🛡️ CORS конфигурация

**Backend (docker-compose.prod.yml):**
```yaml
CORS_ORIGINS: '["https://justspeak.heliad.ru"]'
```

**Что это значит:**
- Backend принимает запросы **только** с домена `https://justspeak.heliad.ru`
- Запросы с других доменов блокируются
- Защита от несанкционированного доступа

## ✅ Проверка правильности конфигурации

### 1. Frontend доступен
```bash
curl -I https://justspeak.heliad.ru
# Ожидается: HTTP/1.1 200 OK
```

### 2. Backend API доступен
```bash
curl https://justspeak.heliad.ru/health
# Ожидается: {"status":"healthy"}
```

### 3. Порты закрыты снаружи
```bash
curl http://158.160.141.83:3005
# Ожидается: timeout или connection refused

curl http://158.160.141.83:8005
# Ожидается: timeout или connection refused
```

### 4. HTTPS редирект работает
```bash
curl -I http://justspeak.heliad.ru
# Ожидается: HTTP/1.1 301 Moved Permanently
# Location: https://justspeak.heliad.ru/
```

## 🎯 Итоговая схема

```
┌─────────────────────────────────────────────────────────────┐
│                    Внешний мир (Internet)                    │
└─────────────────────────────────────────────────────────────┘
                              ↓
              https://justspeak.heliad.ru (443)
              http://justspeak.heliad.ru (80 → 301 → 443)
                              ↓
┌─────────────────────────────────────────────────────────────┐
│              Nginx (158.160.141.83:443)                      │
│  - SSL терминация                                            │
│  - Маршрутизация по префиксу                                 │
│  - Проксирование на localhost                                │
└─────────────────────────────────────────────────────────────┘
                    ↓                    ↓
        /api, /health              / (всё остальное)
                    ↓                    ↓
┌──────────────────────┐   ┌──────────────────────────────────┐
│  Backend (FastAPI)   │   │  Frontend (Nginx/React)          │
│  127.0.0.1:8005      │   │  127.0.0.1:3005                  │
│  - API endpoints     │   │  - Статические файлы             │
│  - Обработка данных  │   │  - HTML, CSS, JS                 │
└──────────────────────┘   └──────────────────────────────────┘
          ↓
┌──────────────────────┐
│  PostgreSQL          │
│  127.0.0.1:5435      │
│  - База данных       │
└──────────────────────┘
```

## 📌 Ключевые моменты

1. **Всё доступно только через один домен**: `https://justspeak.heliad.ru`

2. **Frontend не знает точный адрес backend**:
   - Использует относительные пути (`/api`)
   - Браузер сам подставляет домен
   - Nginx проксирует на нужный сервис

3. **Безопасность**:
   - Все внутренние сервисы закрыты (127.0.0.1)
   - Доступ только через Nginx
   - SSL для всех соединений
   - CORS ограничен одним доменом

4. **Простота разработки**:
   - Локально: `docker-compose.yml` открывает порты
   - Production: `docker-compose.prod.yml` закрывает порты
   - Frontend код не меняется (использует `/api`)

5. **Масштабируемость**:
   - Легко добавить CDN
   - Легко добавить load balancer
   - Легко сменить домен (нужно только обновить nginx и CORS)
