...
### 159. Дерево курсов — показывать все уроки секции (11 марта 2026)

- **Исправлено**: преподаватели при прикреплении материалов видели 31 из 37 уроков Beginner (и аналогично в других курсах)
- **Корневая причина**: 6 уроков привязаны напрямую к секции (legacy `section_id`), а 31 — через topics; код `if not section_item.children` показывал legacy уроки **только** если у секции нет topics
- Убрано ограничение — legacy уроки показываются всегда, фильтр `not lesson.topic_id` предотвращает дублирование с уроками из topics
- **Файлы**: `courses.py`

### 158. Перевод по наведению — прокинут API-ключ Yandex (11 марта 2026)

- **Исправлено**: при наведении на слово tooltip показывал «...» в верхнем левом углу и перевод не появлялся
- **Корневая причина**: `YANDEX_DICT_API_KEY` отсутствовал в `docker-compose.prod.yml` — ключ был в `.env`, но не прокидывался в контейнер backend
- Добавлен `YANDEX_DICT_API_KEY` в `docker-compose.prod.yml`
- Исправлено позиционирование loading-индикатора «...» — теперь у курсора, а не в координатах 0,0
- **Файлы**: `docker-compose.prod.yml`, `TranslationTooltip.tsx`

### 157. Matching UX — сдвиг сопоставленных элементов (11 марта 2026)

- **Новый layout**: вместо двух отдельных колонок — строки (left + right рядом)
- При сопоставлении правый элемент перемещается рядом с левым в одну строку
- Несопоставленные правые элементы остаются в пуле внизу
- Клик по сопоставленному правому элементу — разъединяет пару (возврат в пул)
- Подсказка «← выберите справа» при выделенном левом элементе
- **Файлы**: `BlockRenderer.tsx`

### 156. Matching UX — цветовые метки пар + кнопка «Завершить» live-сессии (11 марта 2026)

- **Matching**: каждая сопоставленная пара получает уникальный цвет (10 цветов: синий, зелёный, жёлтый, розовый и т.д.)
- Ученик сразу видит что с чем сопоставлено вместо одинакового синего
- **Live session**: кнопка «Завершить» теперь сразу навигирует на `/schedule` (ранее зависела от WebSocket-сообщения `session_end`, которое могло не дойти)
- **Файлы**: `BlockRenderer.tsx`, `LessonPreviewPage.tsx`

### 155. drag_words и matching — поддержка дублирующихся слов (11 марта 2026)

- **Исправлено**: при одинаковых словах в пуле (два "in", True/False в matching) размещение одного зачёркивало все дубликаты
- **Исправлено**: кнопка «Проверить» не работала у студентов в drag_words (allFilled использовал `w.index`, отсутствующий после strip)
- **Корневая причина**: отслеживание по строковому значению слова вместо уникального индекса
- drag_words: `selectedPoolIndex` + `placedPoolIndices` вместо string-based tracking
- matching: `matchedRightIndices` с `origIndex` вместо `Object.values(matches).includes(right)`
- `allFilled` теперь считает пропуски из текста (`text.match(/\{\d+\}/g)`) вместо `words.every(w => answers[w.index])`
- **Файлы**: `BlockRenderer.tsx`

### 154. drag_words — показывать слова студентам в пуле (11 марта 2026)

- **Исправлено**: студенты видели пустой пул слов в блоках drag_words (пропуски без слов для перетаскивания)
- **Корневая причина**: `strip_answers_from_content()` удаляла поле `word`, оставляя только `index` — фронтенд строил пул из `words.map(w => w.word)`, получая пустой массив
- Теперь слова сохраняются (`word`), но убирается привязка `index→word` (студент не знает какое слово в какой пропуск), массив перемешивается на сервере
- Серверная проверка (`grade_answer_detailed`) не затронута — использует оригинальный контент из БД
- **Файлы**: `courses.py`

### 153. Standalone ДЗ шаблоны не должны автоназначаться (10 марта 2026)

- **Исправлено**: при создании нового шаблона ДЗ (standalone урок) он сразу показывал привязку к урокам, к которым его не прикрепляли
- **Корневая причина**: автоназначение в `attach_course_material()` находило ВСЕ шаблоны курса (включая standalone) и создавало `HomeworkAssignment` при прикреплении курсового материала к любому уроку
- Автоназначение теперь пропускает standalone уроки (`is_standalone=True`) — они назначаются только вручную
- Убрано создание `HomeworkTemplateItem` для новых standalone шаблонов (не нужны)
- Миграция 037: очистка ошибочных `homework_assignments` и `homework_template_items` для standalone уроков
- **Файлы**: `lessons.py`, `homework_templates.py`, миграция 037

### 152. Публикация всех уроков — дефолт is_published=True (10 марта 2026)

- **Исправлено**: преподаватели видели только 32 из 43 уроков Market Leader Intermediate в дереве курсов
- **Корневая причина**: `is_published` по умолчанию `False` в схеме создания урока/курса — уроки, добавленные через редактор, оставались черновиками
- Изменён дефолт `is_published` с `False` на `True` в `InteractiveLessonBase` и `CourseBase`
- Миграция 036: `UPDATE interactive_lessons SET is_published = true` — опубликованы все существующие уроки
- **Файлы**: `course.py` (schema), миграция 036
...
- В табе «Посещаемость» для каждого студента — кнопка «Удалить будущие уроки» (только admin/manager)
- Вызывает `DELETE /api/lessons/batch-scheduled?student_id=`, с confirm-диалогом
- **Файлы**: `LessonDetailModal.tsx`

#### Синхронизация студентов во ВСЕ уроки группы
- `sync_group_students_to_future_lessons()` → `sync_group_students_to_lessons()`
- При добавлении студента в группу он теперь добавляется во **все** уроки группы (включая прошлые), кроме отменённых
- Удаление по-прежнему защищено проверкой `attendance_status == PENDING`
- **Файлы**: `groups.py`

#### Удаление запланированных уроков при удалении ученика
- При soft-delete студента (`DELETE /api/users/{id}`):
  - Индивидуальные SCHEDULED уроки (без группы, 1 студент) → удаляются полностью (с отвязкой транзакций)
  - Групповые / с другими студентами → студент удаляется из `lesson_students`
- **Файлы**: `users.py`

#### Настраиваемый диапазон часов в расписании
- Два input (от/до) в шапке расписания, по умолчанию 8:00–20:00
- Настройка сохраняется в localStorage (ключи `schedule_hours_main_*` / `schedule_hours_student_*`)
- Работает и в основном расписании (`SchedulePage`), и в расписании ученика (`StudentSchedulePage`)
- **Файлы**: `SchedulePage.tsx`, `StudentSchedulePage.tsx`

### 146. Мягкая проверка баланса + блокировка ссылки для должников (3 марта 2026)

#### Мягкая проверка баланса при создании уроков
- **Убрана жёсткая блокировка** (HTTP 400) создания урока при недостаточном балансе ученика
- Урок теперь **создаётся всегда**, преподаватель получает уведомление в колокольчик (`Notification`, тип `LOW_BALANCE`) с перечислением учеников и их балансов
- Изменено во всех 4 эндпоинтах: `POST /api/lessons`, `POST /api/lessons/batch`, `POST /api/teacher/lessons`, `POST /api/teacher/lessons/batch`
- **Файлы**: `lessons.py`, `teacher_dashboard.py`

#### Блокировка ссылки на урок при недостаточном балансе
- Если `student.balance < lesson_type.price` — ссылка на урок скрыта от ученика (API возвращает `meeting_url: null`)
- Новое поле `balance_insufficient: bool` в схеме `StudentLessonInfo`
- Frontend: оранжевый блок «Пополните баланс для доступа к уроку» вместо кнопки «Подключиться» — в расписании (mobile + desktop) и в модалке урока
- При пополнении баланса ссылка становится доступной автоматически
- Серверная защита: URL не передаётся клиенту, даже через DevTools ссылку не достать
- **Файлы**: `student_dashboard.py`, `dashboard.py` (schema), `StudentDashboardPage.tsx`, `types/index.ts`

### 145. Жёсткая проверка баланса, 3 UI-бага live-сессии, скролл расписания (3 марта 2026)

#### Жёсткая проверка баланса при создании уроков
- **Блокировка создания урока** при недостаточном балансе ученика — HTTP 400 с перечислением учеников и их балансов
- Проверка во всех 4 эндпоинтах: `POST /api/lessons`, `POST /api/lessons/batch`, `POST /api/teacher/lessons`, `POST /api/teacher/lessons/batch`
- Убрана старая мягкая нотификация (уведомление учителю) — заменена на жёсткий блок
- **Файлы**: `lessons.py`, `teacher_dashboard.py`

#### Исправления UI live-сессии
- **Sticky баннер «За мной»**: корневой div Layout имел `overflow-x-hidden`, что по CSS-спецификации переключает `overflow-y: visible → auto` и создаёт scroll-контейнер. Контейнер не скроллится сам (нет max-height) → sticky привязывался к нему и не активировался. Заменён на `overflow-x: clip` (клиппинг без scroll-контейнера). Баннер вынесен за пределы `max-w-5xl` контейнера, адаптивный `top-14 lg:top-0` для мобильного хедера
- **TOC sidebar «Содержание»**: всегда строится по всем блокам урока (а не по текущей странице). Клик по пункту другой страницы переключает страницу и скроллит к блоку. Работает для page_break и авто-пагинации
- **Подсказки ответов для преподавателя**: визуальные зелёные пунктирные рамки до нажатия «Проверить» — fill_gaps (рамка + placeholder с ответом), drag_words (зелёные пропуски), matching (зелёная рамка + блок правильных пар)
- **Popup blocked**: `window.open("...", "_blank")` после async API-вызова блокировался браузером (потеря контекста пользовательского действия). Заменён на `navigate()` из react-router
- **Файлы**: `Layout.tsx`, `LessonPreviewPage.tsx`, `BlockRenderer.tsx`, `LessonDetailModal.tsx`

#### Горизонтальный скролл расписания
- `overflow-x: clip` на `<main>` заменён на `min-w-0` — не блокирует горизонтальный скролл вложенных контейнеров
- `min-w-[900px]` на таблице расписания — гарантирует скролл на узких экранах
- Адаптивный header расписания: `flex-wrap` для мобильных устройств
- **Файлы**: `Layout.tsx`, `SchedulePage.tsx`, `StudentSchedulePage.tsx`

### 144. Конструктор ДЗ для админа, matching layout, переименование уроков (3 марта 2026)

- **Конструктор заданий доступен из админки**: пункт меню «Конструктор заданий» (`/homework/editor`) теперь виден для admin и teacher. Admin/manager видят все standalone уроки (от всех преподавателей), teacher — только свои
- **Matching с изображениями — выравнивание колонок**: обе колонки теперь `flex flex-col` с `flex-1` на кнопках для равной высоты; изображения `object-cover` + `h-32` вместо `object-contain` + `h-24`; текстовые кнопки с `flex items-center` для вертикального центрирования
- **Переименование уроков в редакторе курса**: inline-редактирование названия урока — двойной клик по названию или кнопка-карандаш (оранжевая) в toolbar; Enter для сохранения, Escape для отмены; при редактировании скрываются badges
- **Файлы**: `Layout.tsx`, `homework_lessons.py`, `BlockRenderer.tsx`, `CourseEditorPage.tsx`

### 143. Sentence_choice с inline dropdown в тексте + teaching_guide в live-сессии (2 марта 2026)

- **Переделка sentence_choice**: блок «Выбор предложения» теперь работает по паттерну fill_gaps — текст с пропусками `{0}`, `{1}`, каждый пропуск — inline `<select>` dropdown прямо в тексте
- **Новый редактор**: textarea для текста с плейсхолдерами + секция пропусков с вариантами ответа и radio-кнопками для правильного
- **Новый рендерер**: inline dropdown'ы встроены в текст, подсветка результатов (зелёный/красный), teacher hint (dashed green border + tooltip)
- **Backward compat**: старые блоки без поля `text` рендерятся как раньше (нумерованный список dropdown'ов)
- **Тип данных**: добавлено поле `text?: string` в `SentenceChoiceBlockContent`
- **Исправлено**: блоки `teaching_guide` (Заметка для преподавателя) теперь отображаются для преподавателей/админов/менеджеров в live-сессии и превью урока (ранее были скрыты для всех ролей)
- **Файлы**: `BlockEditor.tsx`, `BlockRenderer.tsx`, `course.ts`

### 142. 10 доработок по уточнениям заказчика (1 марта 2026)

#### Фаза 1 — Quick fixes
- **Баг «Завершить» → доска**: `onSessionEnd` теперь всегда перенаправляет на `/schedule` вместо `navigate(-1)`, устраняя цикл через доску
- **Matching — равная ширина столбцов**: `basis-1/2 min-w-0` на оба столбца, `min-h-[48px]` и `break-words` на кнопки
- **Синхронизация учеников группы**: добавлен `await db.flush()` в конце `sync_group_students_to_future_lessons()` + logging для отладки
- **Whiteboard persistence**: состояние доски сохраняется в `sessionStorage` (ключ `wb_{sessionId}`), восстанавливается при возврате. Очистка при завершении сессии
- **Sticky заголовки расписания**: `<thead>` — `sticky top-0 z-10`, ячейка «Время» — `sticky left-0`, обёртка `overflow-auto max-h-[calc(100vh-200px)]`
- **Sticky toolbar live-сессии**: DrawingOverlay toolbar — `fixed top-16 right-4 z-40` вместо `absolute`, остаётся на экране при скролле

#### Фаза 2 — Средние задачи
- **Авто-транскрипция словаря**: `GET /api/vocabulary/lookup?word=` → Free Dictionary API, debounced автозаполнение транскрипции в редакторе блока vocabulary
- **Несколько предложений в word_order**: поле `sentences: WordOrderSentence[]`, обратная совместимость с `correct_sentence`, редактор с добавлением/удалением предложений, серверная проверка каждого
- **Новый блок sentence_choice**: выбор предложения из dropdown. Миграция 033, модель `SENTENCE_CHOICE`, grading `_grade_sentence_choice`, strip `correct_index`, Editor + Renderer с `<select>`

#### Фаза 3 — Большая фича
- **Конструктор ДЗ (standalone уроки)**: преподаватель может создавать собственные интерактивные уроки (`is_standalone=true`, `topic_id=null`), редактировать их через существующий `LessonEditorPage`, назначать как ДЗ ученикам
  - Backend: миграция 034 (`is_standalone` column), API `POST/GET/PUT/DELETE /api/homework-lessons/` (TeacherUser)
  - Frontend: страница `/homework/editor`, ссылка в навигации для teacher, кнопка назначения ДЗ в LessonDetailModal

## Февраль 2026

### 141. Групповые live-сессии (multi-peer) (28 февраля 2026)
- **Групповые уроки**: live-сессии теперь поддерживают несколько учеников одновременно (не только 1:1)
- **Backend multi-peer**: `student_id` → `student_ids: set[int]`, `student_ws` → `student_wss: dict[int, WebSocket]` — все ученики урока включаются автоматически при создании сессии
- **Broadcast**: учитель → все ученики (доска, рисование, скролл, навигация, медиа, курсор)
- **Независимая работа**: в групповых сессиях каждый ученик работает с упражнениями самостоятельно, ответы НЕ синхронизируются с учителем
- **1:1 без изменений**: индивидуальные сессии работают как раньше (answer sync, read-only зеркало у учителя)
- **Кнопка «Открыть»**: доступна для любых уроков с учениками (убрано ограничение `students.length === 1`)
- **URL**: `&group=1` query-param для группового режима
- **Баннер**: показывает «Учеников: N» для групповых сессий, «Ученик подключён» для 1:1
- **`peersConnected` counter**: на фронтенде отслеживает количество подключённых участников
- **WhiteboardPage**: `wb_open` отправляется при каждом `onPeerJoined` (вновь подключившийся ученик переходит на доску)
- **Cleanup**: 60-сек таймаут после отключения ВСЕХ участников (teacher + все students)
- **CLAUDE.md удалён из git**: файл остаётся локально, добавлен в `.gitignore`

### 140. Интерактивная доска (Whiteboard) для live-сессий (28 февраля 2026)
- **Полноэкранная доска** (`/whiteboard`): отдельная страница для рисования во время live-сессии учитель-ученик
- **Инструменты рисования**: карандаш (freehand), линия, стрелка (с наконечником), прямоугольник, круг/эллипс, текст (multiline), вставка изображения (загрузка в S3), ластик
- **Палитра и толщина**: 6 цветов, 3 толщины линии — выбор в toolbar
- **Undo/Redo**: полноценная отмена/повтор с горячими клавишами Ctrl+Z / Ctrl+Y, очистка всей доски
- **Экспорт PNG**: скачивание содержимого доски как изображения
- **Виртуальная система координат**: фиксированный холст 1920×1080, uniform scale + center для подгонки к экрану — одинаковое отображение на любых устройствах
- **Real-time синхронизация**: WebSocket сообщения `wb_open`, `wb_close`, `wb_add`, `wb_delete`, `wb_clear`, `wb_snapshot` через существующий relay (без изменений бэкенда)
- **Модель**: учитель рисует, ученик видит read-only. При подключении ученика учитель отправляет snapshot текущего состояния
- **Навигация**: кнопка «Доска» в баннере live-сессии (для учителя); ученик автоматически переходит на доску (`wb_open`) и возвращается к уроку (`wb_close`)
- **Эфемерная доска**: без сохранения в БД, состояние хранится в памяти фронтенда
- **`ProtectedFullscreenRoute`**: новый тип маршрута — аутентификация без Layout-обёртки (без sidebar/header)
- **Файлы**: `useWhiteboard.ts` (хук состояния), `WhiteboardCanvas.tsx` (canvas-рендерер), `WhiteboardPage.tsx` (страница), изменены `useLiveSession.ts`, `App.tsx`, `LessonPreviewPage.tsx`

### 139. Пакет улучшений — 10 пожеланий заказчика (28 февраля 2026)
- **Скрыт баланс на дашборде студента** (временно): убраны цифра баланса в шапке, карточка баланса в статистике, секция «Остаток уроков». Экран блокировки при нулевом балансе оставлен. Управляется флагом `HIDE_BALANCE` в `StudentDashboardPage.tsx`
- **Уведомления — раскрытие текста**: длинные уведомления (например, «Низкий баланс учеников») показываются в 2 строки (`line-clamp-2`), клик раскрывает полный текст
- **Имя преподавателя в профиле ученика**: admin/manager видят назначенного преподавателя во вкладке «Информация». Новый эндпоинт `GET /api/users/{id}/assigned-teachers`
- **Фильтр учеников по балансу**: на странице «Пользователи» добавлены поля «Баланс от/до» и сортировка (по имени, по балансу). Новые query-параметры `balance_from`, `balance_to`, `sort_by` в `GET /api/users`
- **Остаток уроков для admin/manager**: в профиле ученика отображаются карточки с расчётом `баланс / цена` по каждому типу урока. Новый эндпоинт `GET /api/users/{id}/remaining-lessons`
- **Sticky sidebar fix**: `overflow-x-hidden` на `<main>` в `Layout.tsx` заменён на `overflow-x: clip` — обрезает содержимое, но не создаёт scroll container, что позволяет sticky работать корректно
- **Подсказки ответов учителю в live session**: до нажатия «Проверить» преподаватель видит правильные ответы (зелёный пунктирный контур) во всех типах упражнений. Новое состояние `correct_hint` в рендерерах
- **Правильные ответы ученику после проверки**: `grade_answer_detailed()` в `grading.py` теперь возвращает расширенные details:
  - test/image_choice: `correct_missed` для невыбранных правильных вариантов
  - fill_gaps/drag_words: `correct_answers` — правильные ответы для неправильных пропусков
  - true_false: `correct_answer` — правильный ответ
  - word_order: `correct_sentence` — правильное предложение
  - matching: `correct_pairs` — правильные пары для ошибочных
- **Шаблоны домашних заданий** (миграция 032): модели `HomeworkTemplate` и `HomeworkTemplateItem`, CRUD API (`/api/homework-templates`, AdminUser). Раздел «Тесты» переименован в «Домашние задания» с UI конструктора шаблонов (выбор курса → чекбоксы уроков). **Автоназначение**: при прикреплении курсового материала к уроку — автоматически создаются HomeworkAssignment по шаблонам
- **Рисование в live session**: компонент `DrawingOverlay` с Canvas 2D API. Преподаватель рисует поверх урока (4 цвета, 3 толщины, очистка), ученик видит в реальном времени через WebSocket (`drawing_stroke`, `drawing_clear`)
- Создано 4 файла: `032_add_homework_templates.py`, `homework_templates.py`, `homework_template.py`, `DrawingOverlay.tsx`
- Изменено 18 файлов
- Коммит: `8850986`
...
- **«За мной» с переключением страниц**: кнопка теперь отправляет номер текущей страницы вместе со скролл-позицией
  - Если ученик на другой странице — сначала переключается страница, затем скроллится к позиции учителя
  - Расширен тип `sendScrollTo(scrollPercent, page?)` в `useLiveSession.ts`
- **Graceful 409**: повторное нажатие «Открыть» при уже активной live-сессии — просто открывает сессию в новой вкладке без ошибки (вместо красного текста «Active session already exists»)
- Изменено 3 файла: `useLiveSession.ts`, `LessonPreviewPage.tsx`, `LessonDetailModal.tsx`
- Коммит: `7a06fed`
