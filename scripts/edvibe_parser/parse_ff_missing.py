"""Parse missing FF Level 1 lessons 20-21."""
import asyncio
import json
import re
...
async def main():
    ...
    try:
        ...
        for idx in missing_indices:
            ...
            # Navigate to book folder directly
            book_url = "https://edvibe.com/cabinet/school/materials/book/198487/content"
        ...
        existing_file = Path(__file__).parent / "output" / "jsi_hierarchy_FF_Level_1_20260206_093711.json"
        with open(existing_file, 'r', encoding='utf-8') as f:
            hierarchy = json.load(f)

        # Convert and append missing lessons
        for lesson_data in lessons_data:
            flat = edvibe.convert_lesson_to_jsi_flat(
                lesson_data,
                lesson_data.get("lesson_title", "")
            )
            hierarchy["sections"][0]["lessons"].append(flat)
            print(f"Appended: {flat['title']} ({len(flat['blocks'])} blocks)")

        # Save updated hierarchy
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = Path(__file__).parent / "output" / f"jsi_hierarchy_FF_Level_1_{timestamp}.json"
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(hierarchy, f, ensure_ascii=False, indent=2)

        total = sum(len(l["blocks"]) for l in hierarchy["sections"][0]["lessons"])
        print(f"\n[OK] Level 1 complete: {len(hierarchy['sections'][0]['lessons'])} lessons, {total} blocks")
