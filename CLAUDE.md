...
- **StudentDashboardPage**:
  ...
  - Мобильное расписание: day picker + карточки уроков вместо таблицы (кликабельные, открывают модалку с деталями и материалами)
...
- **API для преподавателей**: `PUT /api/teacher/lessons/{lesson_id}` (TeacherUser, проверка владения)
- **API для менеджеров/админов**: `PUT /api/lessons/{lesson_id}` (ManagerUser)
- LessonDetailModal автоматически выбирает правильный API в зависимости от роли пользователя
- Студенты видят ссылку в своём расписании как кнопку "Подключиться к уроку"
...
4. **Ученик видит материалы** если назначен на урок (любой статус посещаемости)
...
- `GET /api/student/course-materials` — материалы курсов для ученика (все уроки, где назначен)
- `GET /api/student/course-material/{id}/view` — отфильтрованное дерево курса для ученика (только прикреплённая часть, проверка назначения на урок)
...
- student — просмотр прикреплённых материалов (если назначен на урок, без требования отметки посещаемости), видит только прикреплённую часть курса (не весь курс)
...
- `LessonDetailModal` — детали урока, отметка посещаемости, материалы урока (для teacher используется teacherApi с балансовой логикой, кнопка "Удалить" скрыта для преподавателей)
