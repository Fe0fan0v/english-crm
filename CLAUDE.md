...
- `/api/levels` — уровни
- `/api/settings` — системные настройки (admin CRUD, публичный GET для WhatsApp номера)
- `/api/settings` — настройки системы (GET /public для всех, остальное только admin)
...
- `NewsPage` — новости (заглушка) для учеников
