...
- Модель `TeacherStudent` для индивидуальных учеников, не состоящих в группах
...
- **Преподаватель ↔ Ученик**: Преподаватель может написать ученику из вкладки "Ученики"
- **Менеджер ↔ Ученик**: Менеджеры могут писать ученикам через раздел "Сообщения" (`/messages`)
...
80. **Прямые назначения преподаватель-ученик** — новая модель TeacherStudent (миграция 018) для связи индивидуальных учеников с преподавателями без создания групп, преподаватель видит назначенных учеников во вкладке "Ученики" и может им писать, скрипт import_individual_assignments.py для импорта из файла Индивиды.xlsx
