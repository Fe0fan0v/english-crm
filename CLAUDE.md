...
├── alembic/versions/  # Миграции (000-020)
└── requirements.txt
...
**Курс → Секции → Уроки → Блоки упражнений**
...
3. **Преподаватель прикрепляет материалы** к своим урокам через кнопку "Добавить материал":
   - Можно прикрепить **весь курс** (например, "Beginner")
   - Можно прикрепить **секцию** (например, "A-1")
...
- `InteractiveLesson` — интерактивный урок (section_id, title, description, position, is_published, is_homework)
- `ExerciseBlock` — блок упражнения (lesson_id, block_type, content JSONB, position)
- `LessonCourseMaterial` — связь урока с материалом курса (lesson_id, student_id, course_id/section_id/interactive_lesson_id)
...
- `POST /api/courses/sections/{id}/lessons` — создать урок
- `GET/PUT/DELETE /api/courses/lessons/{id}` — операции с уроком
- `POST /api/courses/sections/{id}/lessons/reorder` — изменить порядок уроков
...
- `AttachCourseMaterialModal.tsx` — выбор курса/секции/урока для прикрепления (teacher)
...
## Миграции (Alembic)
Текущая версия: **020**
- 000: Initial schema (базовые таблицы: users, levels, lessons, materials, etc.)
- 001: Начальная схема
...
- 017: Add created_by_id to transactions (отслеживание автора транзакций)
- 018: Add teacher_students table (прямые назначения преподаватель-ученик)
- 019: Add course constructor (courses, course_sections, interactive_lessons, exercise_blocks)
- 020: Add lesson_course_materials table (привязка курсов/секций/уроков к расписанным урокам)
...
- [x] Конструктор курсов (Course Constructor) — интерактивные учебные материалы с 18 типами блоков
...
13. **Исправлены графики на Dashboard** — теперь включают сегодняшний день (было `range(30)`, стало `range(31)`)
14. **SearchableSelect для преподавателя** — в LessonCreateModal выбор преподавателя теперь с fuzzy search
15. **Повторяющиеся уроки** — batch creation на несколько недель с выбором дней недели
16. **Личные сообщения** — преподаватель↔ученик, раздел "Мои преподаватели" в кабинете ученика
17. **Экспорт отчётов в Excel** — кнопка "Выгрузить в Excel" на странице отчётов
18. **Словарь и Неправильные глаголы** — перенесены из вкладок в пункты меню sidebar
19. **Загрузка файлов в чат** — прикрепление файлов в личных сообщениях и групповом чате (до 10 МБ)
20. **Фото профиля в sidebar** — отображается в левом нижнем углу если загружено
21. **nginx client_max_body_size** — увеличен до 20MB для загрузки файлов
22. **Мобильная версия для учеников** — адаптивный дизайн для iPhone/iPad с гамбургер-меню, мобильным расписанием (day picker + карточки), полноэкранным чатом
23. **Система настроек с WhatsApp** — модель Settings, миграция 013, API endpoints для управления настройками (включая номер WhatsApp менеджера), кнопка "Связаться с менеджером в WhatsApp" при балансе <= 0
24. **Объединены вкладки у преподавателя** — вкладки "Ученики" и "Группы" объединены в одну вкладку "Ученики", отображающую группы и всех учеников
25. **Переименована вкладка у преподавателя** — "Личные материалы" → "Уроки"
26. **База знаний** — новая кнопка в sidebar для преподавателей и учеников (страница-заглушка)
27. **Новости** — новая кнопка в sidebar для учеников (страница-заглушка)
28. **Кнопки "Чат" и "Уроки"** — добавлены напротив групп и индивидуальных учеников у преподавателя
29. **4 папки в разделе Материалы** — "База PDF", "Каталог курсов", "Доска", "Методист" (заглушки)
30. **Страница настроек для админа** — новый раздел /settings в админке для редактирования системных настроек (номер WhatsApp менеджера и др.)
31. **Автоматический логаут при 401** — при невалидном токене автоматически очищается localStorage и перенаправление на /login
32. **Кнопка WhatsApp для студентов** — зелёная кнопка "Связаться с менеджером" в нижней части sidebar (над кнопкой "Выйти")
33. **Полноценная система новостей** — админ создаёт новости с заголовком, текстом, баннером (ссылка или загрузка файла), статусом публикации; ученики видят опубликованные новости
34. **Загрузка баннера для новости** — поддержка как ссылки на изображение, так и загрузки файла (JPG, PNG, GIF, WebP, до 10MB)
35. **Уникальные иконки в меню** — заменена повторяющаяся иконка "графики" для новостей на уникальную иконку "газета"
36. **Система привязки PDF к урокам** — модель LessonMaterial (миграция 015), API endpoints для прикрепления/открепления материалов, компонент AttachMaterialModal с fuzzy search, вкладка "Материалы" в LessonDetailModal, фильтр "База PDF" в MaterialsPage
37. **Production deployment с доменом** — домен justspeak.heliad.ru, SSL сертификат Let's Encrypt (автообновление), nginx reverse proxy с SSL терминацией
38. **Безопасная конфигурация портов** — все сервисы привязаны к 127.0.0.1 (frontend:3005, backend:8005, db:5435), публичный доступ только через nginx (80, 443), CORS настроен только для justspeak.heliad.ru
39. **WebSocket через WSS** — поддержка WebSocket Secure (wss://) через nginx, автоматический выбор протокола (ws/wss) в зависимости от окружения, проксирование Upgrade заголовков
40. **Документация архитектуры** — создана полная документация: ARCHITECTURE.md (роутинг, адресация), WEBSOCKET.md (WebSocket конфигурация), DEPLOYMENT.md (деплой и безопасность), DNS-SSL-SETUP.md (настройка SSL)
41. **CI/CD с production config** — автоматическое копирование docker-compose.prod.yml при деплое, применение миграций, проверка health endpoints
42. **Начальное значение настройки WhatsApp** — миграция 016 автоматически создаёт запись с номером менеджера при первом запуске
43. **Загрузка PDF материалов** — админ может загружать PDF файлы в "База PDF" (до 50 МБ), альтернатива указанию URL
44. **Исправлен баг со списком учеников** — индивидуальные ученики (без группы) теперь отображаются у преподавателя во вкладке "Ученики"
45. **Раздел "Уроки" для ученика** — новая вкладка с материалами к урокам, папки с датами занятий, материалы доступны с момента начала урока и в течение 30 дней
46. **Раздел "Уроки" для преподавателя** — вкладка "Уроки" с папками всех уроков, возможность добавлять материалы из "Базы PDF" (админ загружает PDF в базу, преподаватель выбирает оттуда)
47. **Автоинициализация настроек** — если в таблице settings нет записей, автоматически создаётся настройка whatsapp_manager_phone при первом заходе админа
48. **Исправлен Mixed Content Error** — добавлен `proxy_redirect` в nginx для конвертации HTTP редиректов в HTTPS, решена проблема с FastAPI trailing slash redirects
49. **Преподаватели могут создавать уроки** — разрешено создание уроков преподавателями в своём расписании (ранее было доступно только админам/менеджерам)
50. **Удаление пользователей** — админы могут удалять пользователей на странице UsersPage (soft delete: is_active = False), с подтверждением
51. **Исправлены права доступа к типам занятий** — преподаватели теперь могут читать список типов занятий (GET /api/lesson-types) для создания уроков
52. **Ребрендинг на "Just Speak It"** — изменён заголовок главной страницы с "EngCRM" на "Just Speak It" (title в браузере)
53. **Исправлены права доступа к пользователям и группам** — преподаватели теперь могут читать списки пользователей (GET /api/users) и групп (GET /api/groups) для выбора учеников и групп при создании уроков
54. **Фильтрация уроков по ученику/группе** — в кабинете преподавателя кнопки "Уроки" напротив ученика/группы открывают отфильтрованный список уроков только с этим учеником/группой
55. **Защита от удаления используемых типов занятий** — нельзя удалить тип занятия, если он используется в уроках (выводится сообщение с количеством связанных уроков)
56. **Подсветка уроков на текущую дату** — во вкладке "Уроки" преподавателя занятия на сегодня выделены голубым градиентом и синей рамкой
57. **Фильтры по ученикам и группам** — во вкладке "Ученики" преподавателя добавлены поиск по ученикам (имя/email) и выпадающий список для фильтрации групп
58. **Удалена вкладка "Материалы"** — в кабинете ученика удалена вкладка "Материалы" (остались: Моя страница, Уроки, Тесты, Сообщения)
59. **Прикрепление PDF из базы в чаты** — преподаватели могут прикреплять материалы из "Базы PDF" в личные сообщения и групповой чат (новая кнопка, модальное окно SelectMaterialModal с fuzzy search)
60. **Правильная работа с timezone** — все datetime конвертируются в UTC при сохранении в БД (Pydantic валидаторы в схемах, функция normalize_datetime_to_utc), уроки открываются для учеников ровно в момент начала (без +1 час)
61. **Исправлена ошибка 422 при создании урока преподавателем** — добавлена отдельная схема TeacherLessonCreate без обязательного поля teacher_id для endpoint'а /api/teacher/lessons (teacher_id берётся из current_user)
62. **Исправлено отображение групп преподавателя** — изменено сравнение is_active в SQLAlchemy запросах с `is True` на просто `is_active`, что правильно интерпретируется как SQL условие WHERE is_active = true
63. **Автоматическая очистка ссылок на прошедшие занятия** — фоновая задача APScheduler удаляет meeting_url у завершённых уроков ежедневно в 00:00 UTC, освобождая ссылки Zoom/Meet после окончания занятия
64. **Массовый импорт преподавателей из Excel** — импортировано 89 преподавателей из файла Преподы.xlsx на production с назначением уровней (12+ месяцев / 6-12 месяцев), индивидуальными паролями и ролью teacher
65. **Серверная фильтрация по роли в API users** — добавлен параметр `role` в GET /api/users для фильтрации пользователей по роли на сервере, исправлена проблема с отображением учеников в UsersPage (раньше из-за пагинации ученики не попадали в первую страницу результатов), создан скрипт check_and_fix_students.py для диагностики и активации неактивных учеников
66. **Система автоматических бэкапов БД** — настроена система ежедневных бэкапов PostgreSQL в ps.kz S3 Object Storage (00:00 UTC, хранение 7 дней, автоудаление старых, сжатие gzip), созданы скрипты backup-to-s3.sh и restore-from-s3.sh, подготовлена документация BACKUP.md и инструкция по настройке backup/SETUP.md
67. **Отслеживание автора транзакций** — добавлено поле created_by_id в модель Transaction (миграция 017) для отслеживания кто создал транзакцию (менеджер, преподаватель), все места создания транзакций обновлены для передачи created_by_id
68. **Отчёт по транзакциям учеников** — новая вкладка "Транзакции" в разделе Отчёты с фильтрами (дата с/по), fuzzy поиском (по ученику, описанию, менеджеру), пагинацией, отображением общей суммы, показывает только увеличения баланса (CREDIT транзакции), API endpoint GET /api/reports/transactions
69. **Чаты менеджер-ученик** — менеджеры могут писать ученикам через новый раздел "Сообщения" (/messages), страница ManagerMessagesPage с списком учеников и компонентом DirectChat, добавлен пункт меню "Сообщения" для admin/manager
70. **Ограничение учеников для преподавателя при создании урока** — преподаватель в LessonCreateModal видит только тех учеников, с которыми у него уже были уроки, новые API endpoints: GET /api/teacher/my-students-for-lessons и GET /api/teacher/my-groups-for-lessons, admin/manager по-прежнему видят всех учеников, fuzzy search работает только по доступным ученикам
71. **Фильтрация студентов преподавателя по активности** — во вкладке "Ученики" в кабинете преподавателя отображаются только активные студенты (is_active=True) за всё время работы, исключены неактивные ученики, применено к обоим endpoints: GET /api/teacher/students и GET /api/teacher/students/{teacher_id}
...
78. **CI/CD на новый сервер** — обновлены GitHub Secrets (VPS_HOST=78.40.108.93, VPS_USER=debian, VPS_SSH_KEY), SSH ключ в PEM формате (RSA 4096), деплой через appleboy/ssh-action, автоматический git pull + docker compose build + миграции
79. **Исправлена загрузка в ps.kz S3** — boto3 не работает с ps.kz S3 (ошибка MissingContentLength), реализована загрузка через httpx с ручной подписью AWS Signature V4, файлы успешно загружаются и доступны по публичным URL
80. **Прямые назначения преподаватель-ученик** — новая модель TeacherStudent (миграция 018) для связи учеников с преподавателями, автоматическое создание связи при: создании урока, добавлении ученика в группу, назначении преподавателя группе; преподаватель видит назначенных учеников и может им писать
81. **Ограничение доступа к чатам** — преподаватели могут писать только назначенным ученикам, ученики — только назначенным преподавателям и менеджерам; проверка через TeacherStudent при отправке сообщений и открытии чата
82. **Настройка автоматических бэкапов на production** — обновлён скрипт backup-to-s3.sh для работы через Docker (pg_dump через docker exec), установлены postgresql-client и awscli на сервере, настроена cron задача (00:00 UTC ежедневно), добавлена конфигурация backup-config.env с параметрами подключения к БД через Docker (DB_HOST=127.0.0.1, DB_PORT=5435, DB_CONTAINER=engcrm-db), логирование в /var/log/postgres-backup.log, бэкапы успешно создаются и загружаются в ps.kz S3 (бакет jsi-backups)

## Изменения февраль 2026

83. **Конструктор курсов (Course Constructor)** — полноценная система создания интерактивных учебных материалов (аналог Edvibe):
    - Иерархия: Курс → Секции → Уроки → Блоки упражнений
    - 18 типов блоков: контентные (text, video, audio, image, article, divider, teaching_guide, remember, table, vocabulary) и интерактивные (fill_gaps, test, true_false, word_order, matching, image_choice, flashcards, essay)
    - Backend: модели Course, CourseSection, InteractiveLesson, ExerciseBlock; JSONB для хранения контента блоков; enum ExerciseBlockType
    - Frontend: CoursesPage, CourseEditorPage, LessonEditorPage, LessonPreviewPage; компоненты BlockEditor и BlockRenderer
    - Миграция 019_add_course_constructor с таблицами и индексами
    - **Workflow**: админ создаёт курсы → преподаватель прикрепляет к урокам (весь курс / секцию / урок) → ученик видит только если "Был"
    - **Гранулярность**: можно прикрепить весь курс (Beginner), секцию (A-1) или конкретный урок (Warm Up)
    - **Видимость материалов**: скрываются при статусе "Отмена" или "Неявка"
    - **Напоминание**: у преподавателя индикатор "Ожидается отметка" для прошедших уроков

84. **Прикрепление материалов курсов к урокам (миграция 020)** — новая модель LessonCourseMaterial для связи расписанных уроков с курсовыми материалами:
    - Модель с enum CourseMaterialType (course, section, lesson)
    - GET /api/courses/tree — дерево опубликованных курсов для выбора
    - POST/GET/DELETE /api/lessons/{id}/course-materials — прикрепление/открепление
    - Фильтрация для студентов по attendance_status == PRESENT
    - Компонент AttachCourseMaterialModal — древовидный выбор курса/секции/урока
    - Вкладка "Курсы" в LessonDetailModal
    - Флаг needs_attendance в TeacherLesson для индикатора "Ожидается отметка"

85. **Загрузка файлов для курсов в S3** — возможность загружать изображения и аудио прямо в редакторе курсов:
    - API endpoint: POST /api/uploads/courses (до 50 МБ)
    - Поддержка форматов: jpg, png, gif, webp (изображения), mp3, wav, ogg, m4a (аудио)
    - Компонент FileUploadButton в BlockEditor
    - Обновлены редакторы: ImageEditor, AudioEditor, ArticleEditor, ImageChoiceEditor, FlashcardsEditor
    - Файлы хранятся в S3 бакете jsi/courses/

86. **Деплой конструктора курсов на production** — развёртывание функциональности Course Constructor на lms.jsi.kz:
    ...
    - S3 интеграция включена (S3_ENABLED=true в docker-compose.yml)
    - Обновлён docker-compose.yml с S3 environment variables для backend

87. **Названия блоков упражнений (Block Titles)** — возможность давать названия блокам как на Edvibe:
    ...
    - Нумерация блоков: #{номер} + название

88. **Блок "Словарь" (Vocabulary)** — новый тип блока для списка слов с произношением:
    - Новый тип блока `vocabulary` в enum ExerciseBlockType (миграция 022)
    - Поля: слово (англ.), перевод, транскрипция (опционально)
    - Кнопка произношения через Web Speech API (браузерный синтез речи)
    - Редактор VocabularyEditor с добавлением/удалением слов
    - Рендерер VocabularyRenderer с кнопками воспроизведения

89. **Кнопки перемещения уроков в редакторе курса** — стрелки ↑↓ для изменения порядка уроков в секции:
    - Кнопки появляются при наведении на урок
    - Кнопка "вверх" отключена для первого урока
    - Кнопка "вниз" отключена для последнего урока
    - Автосохранение через API reorder

90. **Поддержка переносов строк в блоке "Заполнить пропуски"** — возможность создавать многострочный текст:
...
91. **Ограничение создания курсов только для админа** — преподаватели больше не могут создавать и редактировать курсы:
...
92. **Прикрепление курсов к урокам из кабинета преподавателя** — две кнопки вместо одной:
