...
46. **Раздел "Уроки" для ученика** — новая вкладка с материалами к урокам, папки с датами занятий, материалы доступны с момента начала урока и в течение 30 дней
47. **Раздел "Уроки" для преподавателя** — вкладка "Уроки" с папками всех уроков, возможность добавлять материалы из "Базы PDF" (админ загружает PDF в базу, преподаватель выбирает оттуда)
48. **Автоинициализация настроек** — если в таблице settings нет записей, автоматически создаётся настройка whatsapp_manager_phone при первом заходе админа
49. **Исправлен Mixed Content Error** — добавлен `proxy_redirect` в nginx для конвертации HTTP редиректов в HTTPS, решена проблема с FastAPI trailing slash redirects
