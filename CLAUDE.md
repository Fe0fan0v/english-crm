...
71. **Фильтрация студентов преподавателя по активности** — во вкладке "Ученики" в кабинете преподавателя отображаются только активные студенты (is_active=True), из индивидуальных уроков берутся только занятия за последние 90 дней (исключены старые неактивные ученики), применено к обоим endpoints: GET /api/teacher/students и GET /api/teacher/students/{teacher_id}
