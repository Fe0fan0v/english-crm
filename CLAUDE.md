...
**Смешанная структура**: уроки могут быть привязаны к секции как через topic (`topic_id`), так и напрямую (`section_id`, legacy). Дерево курсов (`/courses/tree`) показывает оба типа — фильтр `not lesson.topic_id` предотвращает дублирование.
